...
if __name__ == '__main__':
    datasetPath = "/data/00.Data/Sentinel-2-Ship"  # Levir-Ship Yolo-format dataset path
    saved_coco_path = "/data/00.Data/Sentinel-2-Ship"

    total_yolo_annotations, train_keys, val_keys, test_keys = {}, [], [], []
    "Train"
    for i in glob(datasetPath + "/train/anns/*.txt"):
    ...
    "Val"
    for i in glob(datasetPath + "/valid/anns/*.txt"):
        key = os.path.basename(i).replace("txt", "png")
        value = np.loadtxt(i, ndmin=2)
        for n, i in enumerate(value):
            i[0] = 1
    ...
    l2c_train = Yolo2CoCo(image_dir=os.path.join(datasetPath, "train/imgs/"), total_annos=total_yolo_annotations)
    train_instance = l2c_train.to_coco(train_keys)
    l2c_train.save_coco_json(train_instance, os.path.join( datasetPath , 'instances_train2017.json'))

    "Transform Val Dataset"
    l2c_val = Yolo2CoCo(image_dir=os.path.join(datasetPath, "valid/imgs/"), total_annos=total_yolo_annotations)
    val_instance = l2c_val.to_coco(val_keys)
    l2c_val.save_coco_json(val_instance,  os.path.join( datasetPath , 'instances_val2017.json'))
